...
fn validate(handler: &Handler<'_>, method: &Method, params: &Value) -> Result<(), NncError> {
    ...
    if matches!(handler.protocol(), Protocol::Wallet) {
        ...
        return match wallet_method {
            ...
            crate::nwc::WalletMethod::MakeHoldInvoice => check::<w::MakeHoldInvoiceRequest>(params),
            ...
            crate::nwc::WalletMethod::Pay => check::<w::PayRequest>(params),
            crate::nwc::WalletMethod::Receive => check::<w::ReceiveRequest>(params),
